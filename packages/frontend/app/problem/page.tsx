...
export default function ProblemPage() {
    ...
    const fetchProblemData = async () => {
      ...
      try {
        ...
        if (!problemRes.success) {
          throw new Error(problemRes.error || "Failed to fetch problem details from storage");
        }
        const problemData: Problem = problemRes.data;
  ...
  const checkStatus = async (submissionId: string) => {
    try {
        const res = await api.submissions.getById(submissionId);
        if (!res.success) {
        ...
        } else if (data.status === 'Processing' || data.status === 'Running' || data.status.includes('Testing case')) {
          setSubmissionStatus(`🔄 ${data.status} (${data.passed_test_cases || 0}/${data.total_test_cases || 0} passed so far)`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!result.success) {
        throw new Error(result.error || "Submission failed due to a server error.");
      }
      setSubmissionStatus(`Submission created! Status: ${result.data.status}. Now checking for updates...`);
