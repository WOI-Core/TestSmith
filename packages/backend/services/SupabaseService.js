...
class SupabaseService {
    ...
    async uploadProblemFiles(problemId, problemName, markdownContent, solutionCode) {
        ...
        const cleanProblemId = slugifyTaskName(problemId);
        const cleanProblemName = slugifyTaskName(problemName);
        
        const { error: statementError } = await this.storage
            .from('problems')
            .upload(`${cleanProblemId}/Problems/${cleanProblemName}.md`, markdownContent, {
                contentType: 'text/markdown',
                upsert: true
            });

        if (statementError) {
            throw new Error(`Failed to upload problem statement (${cleanProblemId}/Problems/${cleanProblemName}.md) for ${cleanProblemId}.`);
        }

        const { error: solutionError } = await this.storage
            .from('problems')
            .upload(`${cleanProblemId}/Solutions/${cleanProblemName}.cpp`, solutionCode, {
                contentType: 'text/plain',
                upsert: true
            });

        if (solutionError) {
            throw new Error(`Failed to upload solution code (${cleanProblemId}/Solutions/${cleanProblemName}.cpp) for ${cleanProblemId}.`);
        }
    }
    ...
    async uploadProblemConfig(problemId, configContent) {
        ...
        const { error: configError } = await this.storage
            .from('problems')
            .upload(`${cleanProblemId}/config.json`, configContent, {
                contentType: 'application/json',
                upsert: true
            });

        if (configError) {
            throw new Error(`Failed to upload problem config for ${cleanProblemId}.`);
        }
    }
    ...
    async downloadProblemFile(problemId, fileName, isSolution = false) {
        ...
        if (fileName === 'config.json') {
            filePath = `${cleanProblemId}/config.json`;
        } else if (isSolution) {
            filePath = `${cleanProblemId}/Solutions/${fileName}`;
        } else {
            filePath = `${cleanProblemId}/Problems/${fileName}`;
        }
    ...
    async listFilesInFolder(folderPath) {
        // If folderPath is a problem ID (no slashes), ensure consistent naming
        if (!folderPath.includes('/')) {
            folderPath = slugifyTaskName(folderPath);
        }
