...
class ProblemController extends BaseController {
  ...
  getAll = async (req, res) => {
    console.log('[Controller] ProblemController.getAll called.');
    try {
      const problems = await this.supabaseService.getProblemList();
      console.log(`[Controller] Successfully found ${problems.length} problems. Sending 200 OK.`);
      this.ok(res, problems);
    } catch (error) {
      console.error('[Controller] An error occurred in getAll:', error);
      this.internalServerError(res, error);
    }
  }
}
