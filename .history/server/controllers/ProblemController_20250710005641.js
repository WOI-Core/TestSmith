...
class ProblemController {
    ...
    static async syncWithSearchsmith(req, res, next) {
        try {
            ...
            // Assuming SearchSmith response contains tags and embedding_vector in result
            const { tags, embedding_vector } = searchsmithResponse.data.result;

            await problemRepo.updateProblem(problem_id, {
                is_tagged: true,
                tags: tags || [], // Ensure tags is an array, even if null from SearchSmith
                embedding_vector: embedding_vector || null // Ensure embedding_vector is handled
            });
