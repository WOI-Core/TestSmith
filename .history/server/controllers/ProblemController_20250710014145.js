...
class ProblemController {
    ...
    static async createProblem(req, res, next) {
        try {
            ...
            if (newProblem && newProblem.problem_id) {
                // Use problemData.name to construct filenames for upload
                const problemFileName = newProblem.problem_name || problemData.name; // Use DB name or fallback to submitted name
                
                await supabaseService.uploadProblemFiles(
                    newProblem.problem_id,
                    `${problemFileName}.md`, // Consistent naming for markdown
                    problemData.statement,
                    `${problemFileName}.txt`, // Consistent naming for solution (using .txt as a generic type)
    ...
    static async getProblemDetailsFromStorage(req, res, next) {
        try {
            ...
            const problemNameFromConfig = problemConfig.title; // Get name from config.json

            // Attempt to download markdown and solution based on problem name from config
            const markdownContent = await supabaseService.downloadProblemFile(problem_id, `${problemNameFromConfig}.md`);
            const solutionCode = await supabaseService.downloadProblemFile(problem_id, `${problemNameFromConfig}.cpp`); // Assuming .cpp based on screenshot
