...
class ProblemController {
    ...
    // New method to get problems from storage bucket
    static async getProblemsFromStorage(req, res, next) {
        try {
            const supabaseService = new SupabaseService();
            const problems = await supabaseService.getProblemList(); // Fetch from storage
            res.status(200).json(problems);
        } catch (error) {
            next(error);
        }
    ...
    static async syncWithSearchsmith(req, res, next) {
        try {
            const { problem_name, problem_id } = req.body;
            const problemRepo = new ProblemRepository(supabase);
            const supabaseService = new SupabaseService();
